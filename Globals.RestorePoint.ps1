...
function Read-ModParam
{
	...
	$settingpath = "ModSettings.json"
	$hashtable = Get-Settings -settingname $settingpath
	$value = $hashtable[$key]
	return $value
}
function Write-ModParam
{
	...
	if ($Append)
	{
		$oldvalue = Read-ModParam -settingname $settingpath -key $key
		$value = $oldvalue + ';' + $value
	}
	Set-Settings -settingname $settingpath -key $key -value $value
}
